        ...
        @foreach ($documents as $document)
            <div class="bg-white px-4 rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-auto dark:border-gray-800 dark:bg-gray-800">
                ...
                <div class="flex space-x-1">
                    <a href="{{ route('documents.show', $document->id) }}" class="p-1 text-gray-400 hover:text-blue-600 rounded-full hover:bg-blue-50 transition-colors dark:hover:bg-gray-700" title="Voir">
                    ...
                    <form action="{{ route('documents.destroy', $document->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce Document ?');">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="p-1 text-gray-400 hover:text-red-600 rounded-full hover:bg-red-50 transition-colors dark:hover:bg-gray-700" title="Supprimer">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                ...
                <div class="px-3 py-2 pt-3 flex justify-center gap-2 border-t dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <a href="{{ route('documents.show', $document->id) }}" 
                        class="inline-flex items-center px-2 py-1 border border-gray-500 rounded-xl text-xs font-medium text-gray-500 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-white/[0.05]">
