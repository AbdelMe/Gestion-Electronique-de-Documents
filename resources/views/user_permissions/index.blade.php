...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('user_permissions.assignPermitionsToRole') }}"
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-2">
                                        @forelse($role->permissions as $permission)
                                            <div class="group relative">
                                                <span
                                                    class="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 flex items-center gap-1">
                                                    {{ $permission->name }}
                                                    <button type="button"
                                    ...
                                    @if ($role->permissions->count() > 0)
                                        <x-confirm-button message='Are you sure you want to revoke All permissions?'
                                            action="{{ route('user_permissions.revokeAllPermissions', $role->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')

                                            <button type="submit"
                                                class="flex items-center text-xs text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
                                        ...
                                        {{-- <form method="POST"
                                            action="{{ route('user_permissions.revokeAllPermissions', $role->id) }}"
                                            class="mt-2"
                                            onsubmit="return confirm('Are you sure you want to revoke All permissions?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="flex items-center text-xs text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
