    ...
    <div class="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
        <div
            ...
            {{-- <button
                ...
                <svg x-show="!sidebarToggle" class="fill-current" width="24" height="24" viewBox="0 0 24 24"
                ...
                <svg x-show="sidebarToggle" class="fill-current" width="24" height="24" viewBox="0 0 24 24"
                    fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                        d="M18.3 5.71a1 1 0 00-1.41 0L12 10.59 7.11 5.7a1 1 0 10-1.41 1.42L10.59 12l-4.9 4.89a1 1 0 101.41 1.42L12 13.41l4.89 4.9a1 1 0 001.42-1.42L13.41 12l4.9-4.89a1 1 0 000-1.4z"
                        fill="currentColor" />
                </svg>
            ...
            <a href="{{ route('dashboard') }}" class="lg:hidden flex items-center">
                ...
                <h1 class="text-3xl font-bold tracking-widest ml-2">A R C H I V I</h1>
            </a>
        ...
        <div :class="menuToggle ? 'flex' : 'hidden'"
            ...
            <div class="2xsm:gap-3 flex items-center gap-2">
                {{-- Translating --}}
                <div class="relative" x-data="{ langDropdownOpen: false }" @click.outside="langDropdownOpen = false">
                    <button
                        class="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
                        @click.prevent="langDropdownOpen = !langDropdownOpen">
                ...
                <div class="relative" x-data="{ dropdownOpen: false, notifying: true }" @click.outside="dropdownOpen = false">
                    ...
                    <div x-data="{ dropdownOpen: false }" class="relative">
                        <div x-data="{ dropdownOpen: false, notifying: true }" class="relative">
                            <button
                                class="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
                            ...
                            <div x-show="dropdownOpen" x-cloak @click.outside="dropdownOpen = false"
                                @class([
                                    'fixed z-[9999] mt-[5px] flex h-[480px] w-[350px] flex-col rounded-2xl border p-3 sm:w-[361px]',
                                    'right-10' => !$isRtl,
                                    'left-0' => $isRtl,
                                    'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-800 shadow-theme-lg ',
                                ])
                                x-transition:enter="transition transform ease-out duration-300"
                                x-transition:enter-start="{{ $isRtl ? '-translate-x-20 opacity-0' : 'translate-x-20 opacity-0' }}"
                                x-transition:enter-end="translate-x-0 opacity-100"
                                x-transition:leave="transition transform ease-in duration-200"
                                x-transition:leave-start="translate-x-0 opacity-100"
                                x-transition:leave-end="{{ $isRtl ? '-translate-x-20 opacity-0' : 'translate-x-20 opacity-0' }}">
                                ...
                                <div class="mb-3 flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                                    ...
                                    {{-- <button type="submit" class="inline-flex items-center px-4 text-red-700 border hover:text-white border-red-600 py-1 hover:bg-red-700 dark:text-white text-sm font-medium rounded-xl shadow-sm transition-colors duration-200">
                                        Delete All
                                    </button> --}}
                                ...
                                <ul id="notification-container" class="Hide-scroll custom-scrollbar flex h-auto flex-col overflow-y-auto" style="max-height: 400px;" x-cloak>
                                </ul>

                                <div id="loading" style="display:none;" class="text-center text-sm text-gray-400">Loading...</div>
                                ...
                                <script>
                                    ...
                                    function loadNotifications() {
                                        ...
                                        fetch(`/notifications/load?page=${page}`)
                                            .then(response => {
                                                ...
                                                data.data.forEach(notification => {
                                                    ...
                                                    li.innerHTML = `
                                                        <a class="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5" href="#">
                                                            ${notification.from_user && notification.from_user.profile_photo_path ? `
                                                                <span class="relative z-1 block h-10 w-full max-w-10 rounded-full">
                                                                    <img src="/storage/${notification.from_user.profile_photo_path}" alt="User" class="overflow-hidden rounded-full" />
                                                            ...
                                                            <span class="block">
                                                                ...
                                                                <span class="text-theme-xs flex items-center gap-2 text-gray-500 dark:text-gray-400">
                                                                    <span>${notification.created_at_diff}</span>
                                                                    <form action="/notifications/${notification.id}" method="POST" onsubmit="return confirm('Are you sure you want to delete this notification?');">
                                                                        <input type="hidden" name="_token" value="${csrfToken}">
                                                                        <input type="hidden" name="_method" value="DELETE">
                                                                        <button type="submit" class="p-1 text-gray-400 hover:text-red-600 rounded-full hover:bg-red-50 transition-colors dark:hover:bg-gray-700">
                ...
                <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                    <a class="flex items-center text-gray-700 dark:text-gray-400" href=""
                        ...
                        <span class="mr-3 h-11 w-11 overflow-hidden rounded-full">
                            <img src="{{ Auth::user()->profile_image ? asset('storage/' . Auth::user()->profile_image) : asset('assets\images\icons\user (1).png') }}"
                                alt="User" />
                        </span>

                        <span
                            class="text-theme-sm mr-1 block font-medium {{ app()->getLocale() === 'ar' ? 'px-2' : '' }}">{{ Auth::user()->first_name }}</span>
                    ...
                    <div x-show="dropdownOpen" x-cloak {{-- class="text-sm text-gray-500 dark:text-gray-400 {{ $isRtl ? 'px-4 pt-1' : '' }}" --}}
                        class="shadow-theme-lg dark:bg-gray-dark fixed z-[9999] right-4 mt-2 flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 dark:border-white/[0.03] dark:bg-gray-900 {{ $isRtl ? 'translate-x-24 ' : '' }}">
                        <div>
