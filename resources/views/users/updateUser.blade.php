    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-wrap rounded-xl bg-white dark:bg-transparent">
            <div
                ...
                <div class="mt-4 text-lg font-semibold dark:text-gray-300">{{ $user->first_name }}</div>
                <div class="text-sm text-gray-500 dark:text-gray-300">{{ $user->email }}</div>
            </div>

            <div class="w-full md:w-2/3 p-6">
                <form method="POST" action="{{ route('users.saveUpdatedUser' , $user->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="mb-6 flex justify-between items-center">
                        <h4 class="text-2xl font-semibold text-gray-800 dark:text-gray-300">Update User</h4>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Block</label>
                                ...
                                <select name="blocked"
                                    ...
                                    <option value="">-- Sélectionnez une block --</option>
                                    <option value="{{ 1 }}" {{ $user->blocked == 1 ? 'selected' : '' }}>
                                        Block
                                    </option>
                                    <option value="{{ 0 }}" {{ $user->blocked == 0 ? 'selected' : '' }}>
                                        Unblock
                                    </option>
                                    <option value="{{ 0 }}" {{ $user->blocked == 0 ? 'selected' : '' }}>
                                        Not blocked
                                ...
                                <select name="entreprise_id"
                                    ...
                                    @foreach ($entreprises as $entreprise)
                                        <option value="{{ $entreprise->id }}"
                                            {{  old('entreprise', $user->entreprise_id) == $entreprise->id ? 'selected' : '' }}>
