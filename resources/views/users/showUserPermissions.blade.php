...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-800 dark:text-white">Users Permissions</h1>
            <a href="{{ route('users.assignPermitionsToUser') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($users as $user)
                            <tr>
                                ...
                                <div class="group relative">
                                    ...
                                    <form id="remove-permission-{{ $permission->id }}" 
                                          action="{{ route('users.deleteUserPermition', [$user->id, $permission->id]) }}" 
                                          method="POST" 
                                          class="hidden">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <form method="POST" action="{{ route('users.revokeAllUserPermissions', $user->id) }}" 
                          ...
                          @endif --}}
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @forelse($user->getAllPermissions() as $permission)
                                            @php $isDirect = in_array($permission->id, $directPermissions); @endphp
                    ...
                    {{ $isDirect
                        ...
                        : 'bg-red-100 text-red-800 cursor-not-allowed' }}">
                                                    ...
                                                    {{!$isDirect ? $permission->name . " (Role)" : $permission->name }}
                                        ...
                                        @empty
                                            <span class="text-sm text-gray-500 dark:text-gray-400">No permissions
                                                assigned</span>
                                        @endforelse
                                    </div>

                                    @if ($user->getAllPermissions()->count() > 0 && $isDirect)
                                        <form method="POST"
                                            action="{{ route('users.revokeAllUserPermissions', $user->id) }}" class="mt-2"
                                            onsubmit="return confirm('Are you sure you want to revoke all direct permissions?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="flex items-center text-xs text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 mt-2">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        {{-- <a href="{{ route('users.editUserPermissions', $user->id) }}" 
